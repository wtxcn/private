...
(function () {
  ...
  function safeCardName(value) {
    return String(value || "Card").replace(/\d{5,}/g, digits => `...${digits.slice(-4)}`).replace(/\s+/g, " ").trim().slice(0, 100) || "Card";
  }
  ...
  function collectAmexPage() {
    if (currentBank() !== "amex" || !/\/offers/i.test(location.pathname)) return false;
    const rawCardId = new URLSearchParams(location.search).get("opaqueAccountId") || "current";
    const cardName = safeCardName(document.querySelector('[data-testid="simple_switcher_wrapper"]')?.textContent || `Amex Card (...${rawCardId.slice(-4)})`);
    const buttons = Array.from(document.querySelectorAll('button[title="add to list card"]'));
    if (!buttons.length) return false;
    const offers = buttons.map(button => {
      let tile = button;
      for (let depth = 0; depth < 10 && tile; depth += 1, tile = tile.parentElement) {
        if (/view details/i.test(tile.textContent || "") && (tile.textContent || "").length > 30) break;
      }
      const lines = (tile?.textContent || "").split("\n").map(line => line.trim()).filter(Boolean);
      const name = lines.slice(0, 3).join(" | ").replace(/\s+/g, " ").slice(0, 180) || "Amex offer";
      return { key: normalize(name), name, cards: { [rawCardId]: "addable" } };
    });
    ...
    cards.set(rawCardId, { id: rawCardId, name: cardName });
    const merged = new Map((old.snapshot?.offers || []).map(offer => [offer.key, offer]));
    for (const offer of merged.values()) delete offer.cards?.[rawCardId];
    for (const offer of offers) {
      const prior = merged.get(offer.key) || { key: offer.key, name: offer.name, cards: {} };
      prior.name = offer.name;
      prior.cards[rawCardId] = "addable";
      merged.set(offer.key, prior);
    }
    const nextOffers = [...merged.values()].filter(offer => Object.keys(offer.cards || {}).length);
    const priorCard = (old.snapshot?.cards || []).find(card => String(card.id) === rawCardId);
    const priorOffers = (old.snapshot?.offers || []).filter(offer => offer.cards?.[rawCardId]).map(offer => [offer.key, offer.cards[rawCardId]]).sort();
    const nextCardOffers = nextOffers.filter(offer => offer.cards?.[rawCardId]).map(offer => [offer.key, offer.cards[rawCardId]]).sort();
    if (priorCard?.name === cardName && JSON.stringify(priorOffers) === JSON.stringify(nextCardOffers)) return false;
  ...
  function beginDrag(event) {
    ...
    if (!origin?.closest?.("[data-drag], [data-restore]")) return;
